...
def group_id(name):
    df = itchat.search_chatrooms(name=name)[0]
    return df['UserName']
...
def text_reply(msg):
    ...
    #itchat.send('saj', toUserName=itchat.search_friends(nickName='刘')[0]['UserName'])
...
def group_text_reply(msg):
    # 当然如果只想针对@你的人才回复，可以设置
    if msg['isAt']:
        item = group_id('国庆节')  # 根据自己的需求设置
        #item=itchat.search_chatrooms(name='日妈弄')[0]['UserName']
       # print(item)
